...
Face::NfdFace::NfdFace(::ndn::Face& face, const ::nfd::FaceUri& localUri, const ::nfd::FaceUri& remoteUri)
...
void
Face::NfdFace::sendInterest(const Interest& interest)
{
  NS_LOG_DEBUG("<< Interest " << interest);
  m_appFace.m_impl->processInterestFilters(interest);
}

void
Face::NfdFace::sendData(const Data& data)
{
  NS_LOG_DEBUG("<< Data " << data.getName());
  m_appFace.m_impl->satisfyPendingInterests(data);
}
